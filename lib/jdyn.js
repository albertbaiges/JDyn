...
class JDyn {
    ...
    async scan(tableName, filter, projection, options) {
        ...
        if (filter) {
            ({ expressions, attributeNames, attributeValues } = expressionsFromObject(filter));
        }
        ...
        const input = {
            ...
            ExpressionAttributeValues: Object.keys(attributeValues).length ? marshall(attributeValues) : undefined,
    ...
    async batchWrite(operations) {
        const requestItems = operations.reduce((acum, curr) => {


            acum[curr.table] = [{
                // PutRequest: {
                //     Item: curr.put ? marshall(curr.put) : undefined
                // },
                DeleteRequest: {
                    Key: curr.delete ? marshall(curr.delete) : undefined
                }
            }]
